...
function Map:IconsForMap(mapID)
    ...
    for _, e in ipairs(byMap[mapID] or {}) do
        ...
        elseif e.kind == "turnin" then
            ...
            -- ER et dræb/interager-mål i verden (vi kender bare ikke stedet), og
            -- `oi` = quest'en kræver et samle-item (fx 'Entry Into Karazhan':
            -- The Master's Key) uden kendt sted - begge skal IKKE vises som
            -- klar-til-aflevering endnu, da der stadig er et mål at fuldføre.
            show = (state == "complete")
